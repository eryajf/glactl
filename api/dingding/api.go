...
func NewDC(key, secret string) *DC {
	...
	client, err := dingtalk.NewClient(key, secret)
	if err != nil {
		logger.Fatal("init dingding client failed: " + err.Error())
	}
	return &DC{client: client}
}

// GetDepts 获取部门列表
func (d *DC) GetDepts() (result []*DingTalkDept, err error) {
	rsp, err := d.client.FetchDeptList(1, true, "zh_CN")
	if err != nil {
		logger.Error("get dept failed:%v\n", err)
	}
...
func (d *DC) GetUsers() ([]*DeptDetailUserInfo, error) {
	...
	for _, dept := range depts {
		...
		for {
			rsp, err := d.client.GetDeptDetailUserInfo(&r)
			if err != nil {
				return nil, err
			}
